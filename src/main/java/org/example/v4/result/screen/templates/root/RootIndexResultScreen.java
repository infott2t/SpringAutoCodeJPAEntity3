package org.example.v4.result.screen.templates.root;
...
public class RootIndexResultScreen extends JFrame {
    ...
    public RootIndexResultScreen(UtilStaticV4 usv){
        ...
        setVisible(true);
        setResizable(true);
        setTitle("File: index.html | Thymleaf, /templates/"+thymleafInitUrl+"/index.html");
        setBounds(300,300,650,500);

        RootIndexContextStr rootIndexContextStr = new RootIndexContextStr(usv);

        jta.setText(rootIndexContextStr.rootIndexHTMLStr);
        String code = jta.getText();
        String[] directoryUrl = usv.thymleafInitUrl.split("/");

        try{
            String path = "C:\\category\\templates\\firstinstance\\";
